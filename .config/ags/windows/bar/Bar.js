...
function StartSection() {
  const SideBarButton = Widget.Button({
    ...
    onPrimaryClick: () =>
      Utils.exec(`bash -c "${App.configDir}/shared/scripts/sidebar.sh toggle"`),
  });
  ...
  const SearchButton = Widget.Button({
    ...
    onPrimaryClick: () =>
      Utils.exec(
        `bash -c "${App.configDir}/shared/scripts/sidebar.sh toggle-applauncher"`,
      ),
  });
  ...
  const WallpaperButton = Widget.Button({
    ...
    onPrimaryClick: () =>
      Utils.exec(
        `bash -c "${App.configDir}/shared/scripts/sidebar.sh toggle-wallpapers"`,
      ),
  });
    ...
    children: [
      Widget.Revealer({
        ...
        child: Widget.Box({
          ...
          spacing: 8,
          className: "apps",
          children: SystemTrayService.bind("items").transform((apps) =>
            apps.map((app) =>
              Widget.Button({
                className: "app",
                child: Widget.Icon({
  ...
  // This is the icon and wallpaper button container
  return Widget.Box({
    className: "start",
    vpack: "start",
    vertical: true,
...
// This is the window focus icon on the bar
...
function EndSection() {
  ...
  const TimeIndicator = Widget.Button({
    ...
    child: Widget.Label().poll(
      1000,
      (self) => (self.label = Utils.exec(`date +'%H\n%M'`)),
    ),
    onClicked: (self) => self.attribute.menu.toggle(),
  });
  ...
  return Widget.Box({
    ...
    children: [
      Widget.Box({
        ...
        hpack: "center",
        vertical: true,
        children: [AudioControlButton, SystemControlButton, ScreenShotButton],
      }),
      TimeIndicator,
    ],
  });
